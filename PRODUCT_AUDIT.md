...
| ADR-DS-001 | Design system convergence | ✅ new `@repo/design-tokens` SOT (primitives/semantic/typography/spacing/radius/shadows/themes, `css/variables.css`); `packages/design-system` now `@import`s it instead of owning tokens; IBM Plex Sans/Mono replaces Geist (`lib/fonts.ts`, Storybook `preview-head.html`); `apps/mobile/constants/colors.ts` sources real semantic tokens instead of Expo defaults; canonical 4/8/12/16px radius wired (`--ds-radius-*`); real EXECUTAR dark theme defined (disclosed extrapolation, no Blueprint dark ramp exists — see `themes/dark.ts`); every arbitrary hex in `apps/mobile/app/**` replaced with semantic tokens (`packages/design-system/components/ui/chart.tsx`'s Recharts-attribute-selector hexes are a disclosed, intentional exclusion — that file is shadcn-regenerated, matches Recharts' own hardcoded SVG stroke defaults, not a color choice of ours) | ✅ 5 new `@repo/design-tokens` unit tests; full-repo `bun run typecheck` (37/37) and `bunx ultracite check .` still clean after the change | ✅ new `ci.yml` `token-drift` job (`check-drift.ts`) fails the build if `css/variables.css` and `src/*.ts` disagree — verified it actually catches an injected mismatch, not just that it passes; `visual-regression` (Chromatic) job added but skipped until a real `CHROMATIC_PROJECT_TOKEN` exists (`LAUNCH_RUNBOOK.md` §9) — Storybook already has story coverage for the main shadcn components, unchanged by this ADR |
