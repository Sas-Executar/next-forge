  ...
  () => {
    ...
    beforeAll(async () => {
      ...
      const workspace = await database.workspace.create({
        data: { clerkOrgId: `org_next_action_test_${suffix}`, name: "Next Action Test" },
      });
    ...
    test("a task blocked by an incomplete dependency is excluded", async () => {
      ...
      expect(
        whileBlockerRunning.kind === "SELECTED" &&
          whileBlockerRunning.task.id
      ).toBe(blocker.id);
      ...
      expect(
        afterBlockerNotDone.kind === "SELECTED" &&
          afterBlockerNotDone.task.id
      ).not.toBe(blocked.id);
    ...
    test("higher criticality (more blocked dependents) wins the tie-break", async () => {
      ...
      await database.dependency.createMany({
        data: [
          { workspaceId, fromTaskId: dependent1.id, toTaskId: highCriticality.id },
          { workspaceId, fromTaskId: dependent2.id, toTaskId: highCriticality.id },
        ],
      });
    ...
    test("a genuine tie (equal criticality, equal/no deadline) returns TIE", async () => {
      ...
      expect(
        result.kind === "TIE" &&
          result.candidates.map((t) => t.id).sort()
