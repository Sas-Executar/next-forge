 ...
 */
export const canTransitionRoutineStatus = (from: RoutineStatus, to: RoutineStatus): boolean =>
  ROUTINE_STATUS_TRANSITIONS[from].includes(to);

export const canTransitionRunStatus = (from: RunStatus, to: RunStatus): boolean =>
  RUN_STATUS_TRANSITIONS[from].includes(to);

export const canTransitionDeliveryStatus = (from: DeliveryStatus, to: DeliveryStatus): boolean =>
